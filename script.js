...
function autoNextDialogue() {
  if (currentIndex < conversations.length - 1) {
    ...
    autoNextTimeout = setTimeout(autoNextDialogue, 10000);
  } else {
...
nextButton.addEventListener("click", () => {
  if (currentIndex < conversations.length - 1) {
    ...
    currentIndex++;
    playDialogue(currentIndex);
    autoNextTimeout = setTimeout(autoNextDialogue, 10000); // Restart the timeout for the next dialogue
  } else {
    alert("Percakapan Tim 7 selesai, Terima Kasih");
  }
});
...
startButton.addEventListener("click", () => {
  ...
  playDialogue(currentIndex);
  autoNextTimeout = setTimeout(autoNextDialogue, 10000); // Start the auto-next after 20 seconds
});
